...
//		String [] list2Array = stringList.toArray(new String [stringList.size()]);
	...
	public List<String> no4LetterWords(String[] stringArray) {
		...
		for (String letters : stringArray) {
			if (letters.length() != 4) {
				no4LetterWords.add(letters);
			}
		}
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		...
		for (Integer num : intArray) {
			arrayInt2ListDouble.add(num/2.0);
		}
...
//		System.out.println("findLargest: " + findLargest);
...
//		}
		...
		if (listOne.size() >= listTwo.size()){
			...
			for (int j=0; j<listTwo.size(); j++) {
				if ( j < listOne.size()){
					interleaveList.add(listOne.get(j));
				}
